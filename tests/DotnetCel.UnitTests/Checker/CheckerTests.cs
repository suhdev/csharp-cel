...
namespace DotnetCel.UnitTests.Checker;

/// <summary>
/// Functional tests for <see cref="DotnetCel.Checker"/>. Each test parses a small expression and
...
public sealed class CheckerTests
{
    ...
    {
        var parsed = DotnetCel.Parsing.Parser.Parse(source);
        Assert.False(parsed.HasErrors, "parse: " + string.Join("; ", parsed.Diagnostics.Select(static d => d.Message)));
        env ??= CelEnv.NewBuilder().Build();
        var result = DotnetCel.Checker.Check(parsed.Expression!, parsed.SourceInfo, env, parsed.Diagnostics);
